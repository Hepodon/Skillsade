// RclTracking.cpp
// Description: This file implements the RCL Tracking system for a robot,
// including
//              sensor management, obstacle detection, position tracking and
//              syncing using VEX V5 distance sensors. Most functionalities are
//              achieved through basic sensor fusion and intersection math.

#include "RclTracking.hpp"
#include <cmath>

inline double roundTwoPlaces(double x) { return std::round(x * 100) / 100; }

// Line obstacle class
Singly_Linked_List<Line_Obstacle> Line_Obstacle::obstacleCollection =
    Singly_Linked_List<Line_Obstacle>();
Line_Obstacle::Line_Obstacle(double x1, double y1, double x2, double y2,
                             double lifeTimeMs)
...
bool Line_Obstacle::expired() { return lifeTimer.timeIsUp(); }
...
bool Line_Obstacle::isIntersecting(const SensorPose &sp) const {
  ...
  // Solve for t (distance along sensor ray) and u (position along obstacle
  // segment) Formula: P_sensor + t*vA = P_obstacle_start + u*vB
  ...
  // 2. 0 <= u <= 1 (Intersection lies on the actual segment, not just the
  // infinite line)
  return (t > 0 && u >= 0 && u <= 1);
}

// Polygon obstacles
void Line_Obstacle::addPolygonObstacle(
    const std::vector<std::pair<double, double>> &points, double lifeTimeMs) {
  ...
  for (size_t i = 0; i < points.size(); ++i) {
    ...
    new Line_Obstacle(points[i].first, points[i].second, points[next].first,
                      points[next].second, lifeTimeMs);
  }
}

// Circle obstacle class
Singly_Linked_List<Circle_Obstacle> Circle_Obstacle::obstacleCollection =
    Singly_Linked_List<Circle_Obstacle>();
Circle_Obstacle::Circle_Obstacle(double x_, double y_, double r_,
                                 double lifeTimeMs)
    : x(x_), y(y_), radius(r_),
      lifeTimer(lifeTimeMs < 0 ? MAX_OBSTACLE_DURATION : lifeTimeMs) {
...
bool Circle_Obstacle::expired() { return lifeTimer.timeIsUp(); }
...
bool Circle_Obstacle::isIntersecting(const SensorPose &sp) const {
  ...
  // Calculate 't' (the distance along the ray to the point closest to the
  // circle center) t = dot product of (vector to center) and (ray direction)
...
// RCL sensor class
RclSensor::RclSensor(pros::Distance *distSensor, double horizOffset,
                     double vertOffset, double mainAng, double angleTol)
    : sensor(distSensor), mainAngle(mainAng),
      angleTolerance(std::abs(angleTol)) {
  offsetDist = std::hypot(horizOffset, vertOffset);
  offsetAngle = std::fmod(
      ((std::atan2(vertOffset, horizOffset) * 180.0 / M_PI) + 360), 360);
...
void RclSensor::updatePose(const lemlib::Pose &botPose) {
  ...
  if (sp.heading <= 0)
    sp.heading += 360.0; // avoid 0 or negative
...
bool RclSensor::isValid(double distVal) const {
  ...
  if (distVal > 200 && this->sensor->get_confidence() < 60)
    return false; // Invalid confidence
  if (std::abs(std::fmod(this->sp.heading, 90.0)) > angleTolerance &&
      std::abs(std::fmod(this->sp.heading, 90.0)) < (90 - angleTolerance))
    return false; // Limit heading to intervals around 90 degrees to increase
                  // accuracy
  // Detect if the ray is intersecting with any circular obstacles
  for (auto *item : Circle_Obstacle::obstacleCollection) {
    if (item->isIntersecting(sp))
      return false;
  }
  // Detect if the ray is intersectign with any line obstacles
  for (auto *item : Line_Obstacle::obstacleCollection) {
    if (item->isIntersecting(sp))
      return false;
  }
...
std::pair<CoordType, double> RclSensor::getBotCoord(const lemlib::Pose &botPose,
                                                    double accum) {
  ...
  if (wall == 1) {
    type = CoordType::Y;
    res = FIELD_HALF_LENGTH - sinA * val;
  } else if (wall == 2) {
    type = CoordType::X;
    res = FIELD_HALF_LENGTH - cosA * val;
  } else if (wall == 3) {
    type = CoordType::Y;
    res = FIELD_NEG_HALF_LENGTH - sinA * val;
  } else if (wall == 4) {
    type = CoordType::X;
    res = FIELD_NEG_HALF_LENGTH - cosA * val;
  } else
    return {CoordType::INVALID, 0.0}; // Error case

  // Adjust by offset
  double offRad = degToRad(offsetAngle - botPose.theta);
  if (type == CoordType::X)
    res -= std::cos(offRad) * offsetDist;
  else if (type == CoordType::Y)
    res -= std::sin(offRad) * offsetDist;
...
// Main RCL Tracking
RclTracking::RclTracking(lemlib::Chassis *chassis_, int frequencyHz_,
                         bool autoSync_, double minDelta_, double maxDelta_,
                         double maxDeltaFromLemlib_, double maxSyncPerSec_,
                         int minPause_)
    : chassis(chassis_), goalMSPT(std::round(1000.0 / frequencyHz_)),
      minPause(minPause_), maxSyncPT(maxSyncPerSec_ / frequencyHz_),
      minDelta(minDelta_), maxDelta(maxDelta_),
      maxDeltaFromLemlib(maxDeltaFromLemlib_), autoSync(autoSync_),
      latestPrecise{0, 0, 0}, poseAtLatest{0, 0, 0} {}
...
void RclTracking::updateBotPose(RclSensor *sens) {
  if (sens != nullptr && chassis != nullptr) {
    ...
    if (data.first == CoordType::X)
      pose.x = data.second;
    else if (data.first == CoordType::Y)
      pose.y = data.second;
    else
      return;
...
void RclTracking::mainUpdate() {
  ...
  if (RclSensor::sensorCollection.size() > 0) {
    ...
    double diff_from_lemlib = std::hypot(botPose.x - chassis->getPose().x,
                                         botPose.y - chassis->getPose().y);
    ...
    if (!xs.empty()) {
      ...
      if (meanX > FIELD_NEG_HALF_LENGTH && meanX < FIELD_HALF_LENGTH &&
          std::abs(meanX - botPose.x) >= minDelta) {
    ...
    if (!ys.empty()) {
      ...
      if (meanY > FIELD_NEG_HALF_LENGTH && meanY < FIELD_HALF_LENGTH &&
          std::abs(meanY - botPose.y) >= minDelta) {
    ...
    if (updateAfterAccum && std::any_of(accCount.begin(), accCount.end(),
                                        [](int c) { return c > 0; }))
      updateBotPose();
  }
}
void RclTracking::syncUpdate() {
  ...
  if (real_diff <= maxSyncPT) {
    chassis->setPose(currRclPosition.x, currRclPosition.y,
  ...
  // Otherwise, only sync part of the discrepency
    ...
    // Update (Sync)
    chassis->setPose(chassis->getPose().x + x_update,
                     chassis->getPose().y + y_update, chassis->getPose().theta);
...
void RclTracking::lifeTimeUpdate() {
  ...
  while (circle_itr != Circle_Obstacle::obstacleCollection.end()) {
    if ((**circle_itr).expired())
      circle_itr.remove(true);
    else
      ++circle_itr;
  }
...
void RclTracking::mainLoop() {
  ...
  while (true) {
    ...
    if (frequencyTimer.timeLeft() < minPause)
      pros::delay(
          minPause); // Ensure that the loop pauses at least for minPause
    else
      pros::delay(
          frequencyTimer.timeLeft()); // Otherwise, wait for the remaining time
  }
}
