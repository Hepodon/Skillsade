...
template <typename AnyType> class Singly_Linked_List {
...
public:
  ...
  public:
    // Constructor
    explicit Iterator(Singly_Linked_List *list, Node *node)
        : next(node), previous(list->getHead()), list(list), count(0) {}
...
public:
  ...
  static void
  addPolygonObstacle(const std::vector<std::pair<double, double>> &points,
                     double lifeTimeMs = -1);
...
public:
  ...
  RclSensor(pros::Distance *distSensor, double horizOffset, double vertOffset,
            double mainAng, double angleTol = 10.0);
  void updatePose(const lemlib::Pose &botPose);
  bool isValid(double distVal) const;
  std::pair<CoordType, double> getBotCoord(const lemlib::Pose &botPose,
                                           double accum = NAN);
...
// Main RCL Tracking
inline std::vector<RclSensor *> RclSensor::sensorCollection =
    std::vector<RclSensor *>();
class RclTracking {
public:
  RclTracking(lemlib::Chassis *chassis_, int frequencyHz_ = 25,
              bool autoSync_ = true, double minDelta_ = 0.5,
              double maxDelta_ = 4.0, double maxDeltaFromLemlib_ = 10.0,
              double maxSyncPerSec_ = 3.0, int minPause_ = 20);
